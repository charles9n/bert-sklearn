...
def biobert_from_archive(archive_file, cache_dir, logger):
    """
    ...
    serialization_dir = tempdir + "/" + filename

    vocab = serialization_dir + "/vocab.txt"
    bert_config = serialization_dir + "/bert_config.json"
    if filename == 'biobert_v1.1_pubmed':
        ckpt = "/model.ckpt-1000000"
    else:
        ckpt = "/biobert_model.ckpt"
    tf_ckpt = serialization_dir + ckpt

    return vocab, bert_config, tf_ckpt, tempdir


def scibert_from_archive(archive_file, cache_dir, logger):
    """
    ...
    vocab = serialization_dir + "/vocab.txt"
    weights_archive = serialization_dir + "/weights.tar.gz"
    ...
    return vocab, bert_config, pytorch_ckpt, tempdir
